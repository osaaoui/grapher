...
function pointQuadratique() {
	var hPoint = -pente / (2 * exp);
	//-b/2a
	var yPoint = (4 * exp * ordonnee - (pente * pente)) / (4 * exp);
	//(4ac-b²)/4a
	var depP2 = (exp * (hPoint + 1) * (hPoint + 1)) + (pente * (hPoint + 1)) + ordonnee;
	// nouveau point de depart pour "p2"
	point1 = board.create('point', [hPoint, yPoint], {
		style : 6,
		name : 'p1'
	});
	// point sommet
	...
	point2 = board.create('point', [(hPoint + 1), depP2], {
		style : 6,
		name : 'p2'
	});
	//le "p2" est placer sur la courbe à 1 de distance par rapport au sommet(ceci évite les conflits 0/0)
	point2.setAttribute({
		strokeColor : 'blue',
		fillColor : 'yellow',
		size : 4
...
function afficherOrdonnee() {
	...
	if (typeEquation == 0) {
		ord = board.create('point', [0, dynamiqueB()], {
			style : 6,
			fixed : true
		});
		ord.setAttribute({
		...
		ord = board.create('point', [0, dynamiqueC()], {
			style : 6,
			fixed : true
		});
		ord.setAttribute({
...
function axeDeSymetrie() {
	...
	var x = -dynamiqueB() / (2 * dynamiqueA());
	//-b/2a
	var y = (4 * dynamiqueA() * dynamiqueC() - (dynamiqueB() * dynamiqueB())) / (4 * dynamiqueA());
	//(4ac-b²)/4a
...
function affichageEquationLineairePoint(p, t) {
	var stringEquation = board.create('text', [4, -1,
	function() {
		return 'y= ' + dynamiqueA()//(1)
		+ 'x +' + dynamiqueB()
	}], {
		visible : false
	})//(2)
...
function affichageEquationQuadratiquePoint(p1, p2) {
	if (dynamiqueB() < 0 && dynamiqueC() < 0) {
		...
		// affichage fonction avec les points
	} else if (dynamiqueB() < 0 && dynamiqueC() >= 0) {
		var stringEquation = board.create('text', [4, -1,
		function() {
			return 'y= ' + dynamiqueA()//(1)
	...
	if (dynamiqueB() < 0 && dynamiqueC() < 0) {
		document.getElementById('equationGraph').innerHTML = "y = " + dynamiqueA() + 'x² +(' + dynamiqueB() + ')' + '+(' + dynamiqueC() + ')';
		board.on('update', function() {
			document.getElementById('equationGraph').innerHTML = "y = " + dynamiqueA() + 'x² +(' + dynamiqueB() + ')' + '+(' + dynamiqueC() + ')';
		});
	} else if (dynamiqueB() < 0 && dynamiqueC() >= 0) {
...
function dynamiqueC() {
	var valC;
	if (typeEquation == 1) {
		valC = ((point1.Y() * 4 * ((point2.Y() - point1.Y()) / ((point2.X() - point1.X()) * (point2.X() - point1.X()))) + ((-2 * ((point2.Y() - point1.Y()) / ((point2.X() - point1.X()) * (point2.X() - point1.X()) )) * point1.X()) * (-2 * ((point2.Y() - point1.Y()) ) / ((point2.X() - point1.X()) * (point2.X() - point1.X()) )) * point1.X())) / (4 * ((point2.Y() - point1.Y()) / ((point2.X() - point1.X()) * (point2.X() - point1.X()) )))).toFixed(2);
	}
	return valC;
}
