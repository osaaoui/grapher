...
function trace() {
	...
	if (erreur < 0) {
		...
		if (exp == 0) {
			...
			document.getElementById('btnAfficOrd').disabled = false;
			//activation du bouton Ordonnee
			document.getElementById('canonique').disabled = false;
			//activation du bouton canonique
		}
...
function slidesGenerique(pente, ordonnee, exp) {
	sliderA = board.create('slider', [[4, -3], [6, -3], [exp - 4, exp, exp + 4]], {
		name : 'a',
		visible : false
	});
	sliderA.visible(false);
	sliderB = board.create('slider', [[4, -3.5], [6, -3.5], [pente - 4, pente, pente + 4]], {
		visible : false
	});
	sliderB.visible(false);
	sliderC = board.create('slider', [[4, -4], [6, -4], [ordonnee - 4, ordonnee, ordonnee + 4]], {
		visible : false
	});
	...
	var stringEquation = board.create('text', [4, -2,
	function() {
		return 'y= ' + sliderA.Value().toFixed(2) + 'x²' + (sliderB.Value() < 0 ? '' : '+') + sliderB.Value().toFixed(2) + 'x' + (sliderC.Value() < 0 ? '' : '+') + sliderC.Value().toFixed(2)
	}], {
		fontSize : 18,
		visible : true
	});
...
function slidesCanonique(a, h, k) {
	sliderA = board.create('slider', [[4, -3], [6, -3], [a - 4, a, a + 4]], {
		name : 'a',
		visible : true,
		snapWidth : 1
	});
	sliderA.visible(false);
	sliderB = board.create('slider', [[4, -3.5], [6, -3.5], [h - 4, h, h + 4]], {
		visible : true,
		snapWidth : 1
	});
	sliderB.visible(false);
	sliderC = board.create('slider', [[4, -4], [6, -4], [k - 4, k, k + 4]], {
		visible : true,
		snapWidth : 1
	});
	sliderC.visible(false);
	...
	var stringEquation = board.create('text', [4, -2,
	function() {
		return 'y= ' + sliderA.Value().toFixed(2) + '(x' + (sliderB.Value() < 0 ? '+' : '-') + Math.abs(sliderB.Value().toFixed(2)) + ')²' + (sliderC.Value() < 0 ? '' : '+') + sliderC.Value().toFixed(2)
	}], {
		fontSize : 18,
		visible : true
	});
	...
	$("#sliderA1").slider({
		...
		slide : function(event, ui) {
			...
			if (sliderA.Value() != 0) {
				...
				document.getElementById('btnAxeStm').disabled = true;
				//activation du bouton  Axe symétrie
				document.getElementById('btnAfficZero').disabled = true;
				//activation du bouton Les zéros
				document.getElementById('btnAfficOrd').disabled = true;
				//activation du bouton Ordonnee
...
function validation(equation) {
	var valide = -1;
	// on présume aucune erreur
	var test;

	if (( test = valLimit(equation)) >= 0 || ( test = valCaractere(equation)) >= 0 || ( test = valRepetition(equation)) >= 0) {
		valide = test;
	}
	return valide;
}
...
function animerPente() {
	...
	if (sliderA.Value() == 0) {
		p1 = board.create('point', [0, (sliderB.Value() + sliderC.Value())], {
			style : 6,
			name : 'a',
			trace : true,
			color : 'green',
		...
		p2 = board.create('point', [1, (sliderB.Value() + sliderC.Value())], {
			style : 6,
			name : 'b',
			trace : true,
			color : 'green',
		...
		p3 = board.create('point', [0, (sliderC.Value() + 0)], {
			style : 6,
			name : 'o',
			trace : true,
			color : 'green',
		...
		point1 = board.create('point', [0, (sliderB.Value() + sliderC.Value())], {
			style : 6,
			trace : false,
			strokeWidth : 0.1,
			visible : false
		});
		point2 = board.create('point', [1, (sliderB.Value() + sliderC.Value())], {
			style : 6,
			trace : false,
			strokeWidth : 0.1,
			visible : false
		});
		point3 = board.create('point', [0, (sliderC.Value() + 0)], {
			style : 6,
			trace : false,
			strokeWidth : 0.1,
			visible : false
		...
		if (sliderC.Value() < 0) {
			board.on('update', function() {
				document.getElementById('equationEntree').innerHTML = "y = " + sliderB.Value() + 'x + (' + sliderC.Value() + ')';
			});
		} else {
			board.on('update', function() {
				document.getElementById('equationEntree').innerHTML = "y = " + sliderB.Value() + 'x + ' + sliderC.Value();
...
function afficherOrdonnee() {
	...
	ord = board.create('point', [0, sliderC.Value()], {
		style : 6,
		fixed : true
	});
	ord.setAttribute({
	...
	if (sliderA.Value() == 0) {
		if (sliderC.Value() < 0) {
			document.getElementById('ordonneeEquation').innerHTML = "y = " + sliderB.Value() + 'x + (' + sliderC.Value() + ')';
			board.on('update', function() {
				document.getElementById('ordonneeEquation').innerHTML = "y = " + sliderB.Value() + 'x + (' + sliderC.Value() + ')';
			});
		} else {
		...
		if (sliderB.Value() < 0 && sliderC.Value() < 0) {
			document.getElementById('ordonneeEquationQuadratique').innerHTML = "y = " + sliderA.Value() + 'x² +(' + sliderB.Value() + ')' + '+(' + sliderC.Value() + ')';
...
function axeDeSymetrie() {
	...
	var x = -sliderB.Value() / (2 * sliderA.Value());
	//-b/2a
	var y = (4 * sliderA.Value() * sliderC.Value() - (sliderB.Value() * sliderB.Value())) / (4 * sliderA.Value());
	//(4ac-b²)/4a
	pointBas = board.create('point', [x, (y - 6)], {
		style : 6,
		name : "x= " + x.toFixed(2)
	});
	// point sommet
	pointBas.setAttribute({
		strokeColor : 'black',
		fillColor : 'red',
		size : 4
	});
	pointHaut = board.create('point', [x, (y + 10)], {
		style : 6,
		name : "x= " + x.toFixed(2)
	});
	pointHaut.setAttribute({
		strokeColor : 'black',
		fillColor : 'red',
		size : 4
	});
...
function afficherLesZeros() {
	...
	ordZer = board.create('point', [0, (sliderC.Value())], {
		style : 6,
		name : '',
		fixed : true
	});
...
function erase() {
	...
	document.getElementById('btnresetter').disabled = true;
	//activation du bouton Reset
	document.getElementById('btnpente').disabled = true;
	//activation du bouton Pente
	document.getElementById('btnAfficOrd').disabled = true;
	//activation du bouton Ordonnee
	document.getElementById('btnAxeStm').disabled = true;
